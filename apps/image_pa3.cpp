...
static std::unique_ptr<GShader> make_bm_shader(GBitmap bm, GMatrix localInv) {
    auto sh = GCreateBitmapShader(bm, localInv);
    // just to be sure the factory didn't store pointers
    bm.reset();
    localInv = GMatrix::Scale(0, 0);
    if (false) {
        printf("%p%p", &bm, &localInv);
    }
    return sh;
}
...
static void draw_poly_rotate(GCanvas* canvas) {
    ...
    if (tex) {
        shader = make_bm_shader(*tex, GMatrix::Scale(1.0f/50, 1.0f/50));
...
static void draw_bitmap(GCanvas* canvas, const GRect& r, const GBitmap& bm, GBlendMode mode) {
    ...
    auto sh = make_bm_shader(bm, m.inverseOrI());
...
static void draw_mode_sample2(GCanvas* canvas, const GRect& bounds, GBlendMode mode) {
    ...
    auto dstsh = make_bm_shader(dstbm, GMatrix::Scale(bounds.width(), bounds.height()/5.0f).inverseOrI());
    ...
    auto srcsh = make_bm_shader(srcbm, GMatrix::Scale(bounds.width()/5.0f, bounds.height()).inverseOrI());
